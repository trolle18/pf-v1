...
if(!defined('OC_WP_API')){
    define( 'OC_WP_API', 'OCOP_WP_ADDONS_API' );
}
...
if( !defined( 'MIDDLEWARE_URL' ) ) {
    ...
    if( isset( $_SERVER[ OC_WP_API ] ) && $_SERVER[ OC_WP_API ] != '' ) {
        $OCOP_WP_ADDONS_API = $_SERVER[ OC_WP_API ];
    } elseif( defined( OC_WP_API ) && OCOP_WP_ADDONS_API != '' && OCOP_WP_ADDONS_API ) {
        $OCOP_WP_ADDONS_API = OCOP_WP_ADDONS_API;
    } else {
        $OCOP_WP_ADDONS_API = 'http://wpapi.one.com/';
    }
    $OCOP_WP_ADDONS_API = rtrim( $OCOP_WP_ADDONS_API, '/' );
	define( 'MIDDLEWARE_URL', $OCOP_WP_ADDONS_API.'/api/'.$api_version );
}
if( !defined( 'OCOP_WP_CORE_VERSION' ) ) {
	global $wp_version;
	define( 'OCOP_WP_CORE_VERSION' , $wp_version );
}
...
final class ONECOMUPDATER {
    ...
    **/
    public function ocop_plugin_row_meta( $links, $file ) {
    ...
    **/
    private function set_onecom_plugins() {
        ...
        /* Add "View Details" link for all One.com plugins, if not exist */
        add_filter( 'plugin_row_meta', array($this, 'ocop_plugin_row_meta'), 10, 2 );
    }
    ...
    **/
    private function get_onecom_themes_versions( $themes ) {
        ...
        $check_array[ 'wp_version' ] = OCOP_WP_CORE_VERSION;
