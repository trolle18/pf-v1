(function($){
    $(document).ready(function(){
        ...
        // Install Plugin - button overlap fix
        $(".plugin-action-buttons").each(function() { 
            if ($(this).width() > 150) {
                $(this).closest(".one-plugin-card .action-links").css({'position' : 'initial','display' : 'block','margin-left' : '104px'});
                $(this).closest(".plugin-card-top").find('.column-name').css({'margin-right': '0'});
                $(this).closest(".plugin-action-buttons").css({'clear' : 'both','float' : 'none','text-align' : 'left'});
            }
        });
    ...
    function onecom_resize_thickbox(){
        ...
        $("#TB_window").css({'margin-left': '-' + parseInt((TB_WIDTH / 2),10) + 'px', width: TB_WIDTH + 'px'});
        $("#TB_window").css({'margin-top': '-' + parseInt((TB_HEIGHT / 2),10) + 'px', height: TB_HEIGHT + 'px'});
    }
